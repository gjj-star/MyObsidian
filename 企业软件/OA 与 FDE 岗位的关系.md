...
| **SSO/组织对接** | 对接客户的 LDAP（Lightweight Directory Access Protocol，轻量目录访问协议）/AD（Active Directory，活动目录）/钉钉/企业微信，实现统一登录和组织同步 | ⭐⭐⭐⭐ |
| **二次开发/集成** | OA 与 ERP 对接（OA 审批通过后自动在 ERP 生成单据）、WebService/REST API（Application Programming Interface，应用程序接口）联调 | ⭐⭐⭐ |
