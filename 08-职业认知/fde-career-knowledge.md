---
name: fde-career-knowledge
description: FDE（前线部署工程师）岗位知识卡：定义、技术栈、国内岗位切入点与实习练兵法
metadata:
  node_type: memory
  type: reference
---
...
核心能力：**写代码 + 读代码 + 调试生产环境 + 搞定客户**。详细对比见 [[pm-vs-fde-comparison]]。
...
| 2 Linux+网络实操 | 网络诊断、Shell、SSH 隧道、日志 | 4-6 周 |
| 3 容器+基础设施 | Docker、K8s、云服务、CI/CD | 6-8 周 |
| 4 企业集成 | SSO/OAuth/SAML、API 网关、PG、数据格式、TLS | 4-6 周 |
...
技术（Linux/Docker/后端）可快速补齐；**产品思维 + 客户沟通 + AI 落地经验**三合一很难得。个人背景见 [[guojinjia-profile]]；OA 项目是黄金练兵场，见 [[oa-fde-relationship]]。
