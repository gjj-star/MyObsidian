...
**英文**：Agent vs. Workflow（Dify）

## 范式定义

- **Agent/Skill**：LLM（Large Language Model，大语言模型）拿着工具集，自主决定每一步做什么。控制权在模型手里。
...
2. 无限循环——思考-行动之间打转，需 max_steps（最大步数）一刀切
...
- **选型**：结构化合规业务（审批流、数据清洗、SOP）优先 Workflow；开放探索业务（客服对话、研究分析、排障）优先 Agent
- **混合编排**：复杂场景用 Agent 做意图理解与规划，子任务交给 Workflow 稳定执行，兼顾灵活性与可审计性

## 相关记忆

- [[Agent 编排模式]] — Agent 工作流编排模式（批次/状态机/检查点）
- [[OA 中的 Agent 集成]] — OA 中 Agent 集成五级成熟度模型
- [[SOP 定义]] — SOP 定义，Workflow 最适合 SOP 化场景
