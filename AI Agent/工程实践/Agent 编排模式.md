...
从废弃工程文档蒸馏出的 Agent 编排设计模式，封装为 skill（名称：`agent-orchestrator-pattern`），用于把多个 Skill 组装成可靠的多步骤业务链。

**英文**：Orchestrator Pattern（编排器模式）；HITL（Human-in-the-Loop，人在回路）；Checkpoint（检查点）；Bitable（飞书多维表格）
...
- **条件触发 ≠ 自动执行** — 检测到外部条件满足时主动建议，但需人工确认后才执行
...
- [[SOP 转 Skill 方法论]] — 先有拆分，再有编排
- [[Agent 部署清单]] — 编排设计完，需部署清单落地
- [[人在回路 HITL]] — HITL 的理论基础

**Why:** 从真实 WorkBuddy 生产项目（8 SOP → 7 Skill + Orchestrator）中提炼的可迁移架构模式。核心价值在于"批内自动、批间人工"的实践经验，而非理论推演。

**How to apply:** 面对多步骤业务链时，先用批次边界双维度（时间间隔 + 物理动作）做第一刀切分，再设计状态表作为单一真相源（Single Source of Truth），最后硬编码所有检查点。
