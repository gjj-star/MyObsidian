...
## 2026-09 安全事件：开源 11 天爆 5 漏洞

DSH 开源后约 11 天内被曝出 5 个安全漏洞（奇安信 QVD 编号 QVD-2026-57410，CVSS 9 分级高危）。已见报道的利用场景：**"AI 读取网页"能力被用于控制主机**——恶意网页可借 DSH 读取网页的执行链操纵本机，已有用户 **API 额度被盗刷**。这印证了上文判断 3（无沙箱无权限边界 → 供应链攻击面集中于一份权限）：AI 读网页 = 把不可信网页内容接入宿主进程，与 npm preinstall 类供应链攻击同思路。

自托管警示（社区讨论要点）：

- 插件市场类开放生态默认关闭/谨慎评估；只装可审计来源的插件
- 对 AI 可读目录做最小权限：只读挂载（`--bind` 只读 / `--tmpfs` 临时文件系统），读文件夹外文件无提示即是风险面
- 留意插件运行时行为（如"自动查找密钥"类自取动作），宿主能力越大越需审计
...
> 来源：CC 对话记录 conversation-log（2026-08-25）+ B 站视频《DeepSeek Harness 不会是未来｜三天读完源码后的判断》（老周 AgentBuilder，2026-08-17 发布，2026-08-21 剪藏）；安全事件节：B 站视频《DeepSeek Harness 开源 11 天爆 5 个漏洞》（2026-09-03 剪藏）
