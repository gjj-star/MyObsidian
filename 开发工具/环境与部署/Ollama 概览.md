...
**英文**：Ollama；LLM（Large Language Model）
...
- **API 兼容**：提供与 OpenAI API（Application Programming Interface，应用程序接口）兼容的接口，方便开发时切换
...
- 硬件门槛：70B 参数模型需要高端 GPU（Graphics Processing Unit，图形处理器）
- 能力差距：开源模型在复杂推理和代码生成上不如 Claude 4 / GPT-4
- 显存/内存常驻：不像云端用完即释放
- 维护成本：需自己管理模型更新、版本兼容、量化（Quantization）选择
...
- [[Docker 基础]] — 本地模型也可容器化一键部署
- [[涌现 Emergence]] — 模型规模与能力拐点：量化小模型缺乏涌现能力
