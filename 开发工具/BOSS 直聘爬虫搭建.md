...
**英文**：CDP = Chrome DevTools Protocol（Chrome 开发者工具协议）；IPv6 = Internet Protocol version 6（互联网协议第六版）；JD = Job Description（职位描述）

基于 [boss-zhipin-scraper](https://github.com/eatmoreduck/boss-zhipin-scraper) 搭建 BOSS 直聘 FDE 岗位爬虫的环境记录：Python/Chrome CDP 环境、IPv6 兼容修复、Windows 注意事项与首次抓取结果。

### 环境搭建

- **Python**：3.12.10，安装在 `C:\Users\EDY\AppData\Local\Programs\Python\Python312\`
- **工具**：boss-zhipin-scraper 克隆到 `D:/CC/boss-zhipin-scraper/`
...
**How to apply:** 每次需要抓取最新岗位时按上述模板操作；不同城市只需改 `--city` 参数（城市代码见 BOSS 直聘 URL）；详情 JD 完成后分析技能词频指导技术补课方向。关联 [[FDE 职业知识卡]]（FDE 岗位要求与技能缺口）。

## 相关

- [[HTTP 状态码]] — 爬虫需识别 403/429 等反爬状态码
