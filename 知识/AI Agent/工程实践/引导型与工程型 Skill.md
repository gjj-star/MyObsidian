...
工程型（Procedural）skill 解决"怎么做"（How），引导型（Guided）skill 解决"要什么"（What）。

**英文**：Skill（AI Agent 可调用能力封装）；Guided Skill（引导型技能）；Procedural Skill（工程型技能）；Articulation Gap（表达鸿沟）；MAC（Multi-Agent Clarification，多智能体澄清）；ECLAIR（Enhanced CLArification for Interactive Responses）

## 概念分化：Articulation Gap

用户输入"帮我做一份高级感 PPT"时存在 articulation gap（表达鸿沟）——脑子里有模糊想法，却无法翻译成可执行规格。工程型 skill 直接生成（猜测），导致返工；引导型 skill 先通过结构化提问澄清需求，再执行。
...
4. **用户预期错位**：用户把 AI 当执行者，而非需要理解需求的专家
